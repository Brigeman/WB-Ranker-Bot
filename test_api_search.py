#!/usr/bin/env python3
"""
Тестовый скрипт для проверки API поиска на Wildberries
"""
...
async def test_api_search():
    """Тестируем API поиск на WB"""
    
    # Создаем настройки
    settings = Settings()
    
    # Создаем адаптер
    adapter = WBAPIAdapter(settings, logging.getLogger("test"))
    
    try:
        async with adapter:
            print("🚀 Запускаем тест API поиска...")
            print("✅ API адаптер инициализирован")
